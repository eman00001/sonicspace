...
function Scene() {
  ...
  if (!isAuthenticated) {
    return (
      <div>
        <p>Please sign in to access SonicSpace.</p>
      </div>
    );
  }
  ...
  if (userLoading) {
    return <div>Loading your SonicSpace...</div>;
  }
  ...
  return (
    <div className="scene-container" style={{ width: '100%', height: '100%', position: 'relative' }}>
      {!addSongsOpen && (
        <button className="btn btn-primary add-songs-button" onClick={() => setAddSongsOpen(true)}>+ Add Songs</button>
      )}
      <button className="btn btn-ghost sign-out-button" onClick={() => handleSignOut()}>Sign out</button>
      <Canvas shadows>
